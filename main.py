...
class Console:
    """Minimal console wrapper so --help and --mode check work before installs."""
...
def _print_experiments(cfg) -> None:
    ...
    headers = ("Name", "Model", "Method", "Train Data")
    widths = [len(h) for h in headers]
    for row in rows:
        widths = [max(widths[i], len(str(row[i]))) for i in range(len(headers))]

    console.print("Available Experiments")
    console.print(" | ".join(headers[i].ljust(widths[i]) for i in range(len(headers))))
    console.print("-+-".join("-" * width for width in widths))
    for row in rows:
        console.print(" | ".join(str(row[i]).ljust(widths[i]) for i in range(len(headers))))


def _dependency_status(package: str, import_name: str | None = None) -> tuple[str, bool]:
    name = import_name or package
    return package, importlib.util.find_spec(name) is not None
...
def _print_check(label: str, ok: bool, detail: str = "") -> bool:
    ...
    console.print(f"[{marker}] {label}: {status}{suffix}")
    return ok
...
def mode_check(args):
    """Check whether the local environment has what the project needs."""
    load_env_file()
    required = [
        ("omegaconf", None),
        ("PyYAML", "yaml"),
    ...
    console.print("Maritime VHF ASR preflight check")
    console.print("================================")
    checks = [
        _print_check(
            "Python version",
            sys.version_info >= (3, 10),
            f"found {sys.version.split()[0]}; Python 3.10+ recommended",
        ),
        _print_check("Config file", Path(args.config).exists(), args.config),
        _print_check("Requirements file", Path("requirements.txt").exists(), "install with: pip install -r requirements.txt"),
        _print_check("FFmpeg executable", shutil.which("ffmpeg") is not None, "needed by pydub for many audio formats"),
        _print_check("Label Studio API key", bool(os.environ.get("LABEL_STUDIO_API_KEY")), "set LABEL_STUDIO_API_KEY or copy .env.example to .env"),
    ]

    missing = [package for package, import_name in required if not _dependency_status(package, import_name)[1]]
    checks.append(_print_check("Python packages", not missing, "missing: " + ", ".join(missing) if missing else "all imports found"))

    data_paths = [Path("data/real"), Path("data/simulated"), Path("data/combined")]
    existing_data = [str(path) for path in data_paths if path.exists()]
    checks.append(_print_check(
        "Prepared data directories",
        len(existing_data) == len(data_paths),
        "found: " + ", ".join(existing_data) if existing_data else "run label_studio_export.py, then python main.py --mode data",
    ))
    ...
    _print_check(
        ...
        True,
        str(checkpoint_dir) if checkpoint_dir.exists() else "optional until after training",
    )
    ...
    if not os.environ.get("LABEL_STUDIO_API_KEY"):
        console.print("  2. Add your API key: cp .env.example .env, then edit .env")
    console.print("  3. Download data: python label_studio_export.py --api-key $LABEL_STUDIO_API_KEY")
    console.print("  4. Prepare manifests: python main.py --mode data")
    console.print("  5. Train/evaluate an experiment from README.md")
